---
title: LLM Providers
description: Zypher Agent supports multiple large language model providers through a unified interface. This allows you to switch between different LLM providers while maintaining the same agent behavior and API.
---
...
**Supported models:**
- `gpt-5.2` - Latest and most capable GPT model (recommended)
...
- `o4` - Latest reasoning-focused model
...
Cloudflare AI Gateway provides a unified proxy for multiple AI providers with built-in caching, rate limiting, and analytics:

```typescript
import { cloudflareGateway } from '@zypher/agent';

const cfProvider = cloudflareGateway({
  accountId: Deno.env.get('CLOUDFLARE_ACCOUNT_ID')!,
  gatewayName: 'my-gateway',
  model: 'anthropic/claude-sonnet-4', // Format: provider/model
  apiKey: Deno.env.get('ANTHROPIC_API_KEY')!, // API key for the underlying provider
});
```

**Supported provider prefixes:**
- `anthropic/` - Anthropic Claude models (e.g., `anthropic/claude-sonnet-4-5-20250929`)
- `openai/` - OpenAI GPT models (e.g., `openai/gpt-5.2`)
- `google/` - Google Gemini models
- `grok/` - xAI Grok models
- `deepseek/` - DeepSeek models

**Benefits:**
- **Unified endpoint** - Single gateway for multiple providers
- **Built-in caching** - Reduce costs with automatic response caching
- **Analytics** - Monitor usage across all providers
- **Rate limiting** - Protect against overuse
- **Failover** - Automatic fallback between providers

For setup instructions, see [Cloudflare AI Gateway documentation](https://developers.cloudflare.com/ai-gateway/).
